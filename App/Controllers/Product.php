    ...
    */
    ...
    public function contactAction()
    {
        ...
        try {
            ...
            // Log l'erreur et affiche la page d'erreur 500 si la récupération échoue
            error_log("ERROR in contactAction: Failed to retrieve article ID $id: " . $e->getMessage());
            View::renderTemplate('Error/500.html');
            return;
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Si aucune erreur de validation, simuler l'envoi et rediriger
            if (empty($errors)) {
                // Pour un projet scolaire où mail() n'est pas configuré,
                // on simule le succès de l'envoi d'email.
                $mailSentSuccessfully = true; // <-- SIMULATION ICI

                if ($mailSentSuccessfully) {
                    // Redirection vers la page d'accueil
                    header('Location: /');
                    exit; // Très important : arrête l'exécution du script après une redirection
                } else {
                    // Ce bloc ne devrait pas être atteint avec la simulation $mailSentSuccessfully = true
                    $errors[] = "Erreur lors de l'envoi. Veuillez réessayer.";
                }
            }
        }

        // Afficher la vue du formulaire de contact si la requête est GET ou si des erreurs de validation POST subsistent
        View::renderTemplate('Product/Contact_form.html', [
            'article' => [
                'id' => $article['article_id'],
                'name' => $article['article_name']
